/*
  ...
  Description   : Reads DHT22 (Temp/Humidity), Soil Moisture (Analog), and simulates
                  NPK/pH/EC from RS485 or analog sensors. Sends JSON telemetry to the
                  Farm Bot PC dashboard via HTTP POST over WiFi. Controls water pump
                  relay based on commands from the server.
...
const char* SERVER_URL = "http://192.168.1.100:5000/api/telemetry";  // ← Update IP!
...
// Calibrate these values by testing your sensor in dry air vs. submerged in water
...
// Standard command for Chinese RS485 NPK sensor (Model: JXBS-3001-NPK)
...
void loop() {
  ...
  if (now - lastTelemetrySendTime >= TELEMETRY_INTERVAL_MS) {
    ...
    sendTelemetryToServer(temperature, humidity, soilMoisture,
                          nitrogen, phosphorus, potassium, soilPH, soilEC);
  }
}
...
void sendTelemetryToServer(float temp, float hum, float moisture,
                           int n, int p, int k, float ph, float ec) {
...
int readNPK_N() {
  ...
  if (val >= 0) return (response[3] << 8) | response[4];  // High + Low byte
...
int readNPK_P() {
  byte response[11];
  int val = readRS485Sensor(NPK_CMD, sizeof(NPK_CMD), response);
  if (val >= 0) return (response[5] << 8) | response[6];
  return 30;  // Fallback
}
...
int readNPK_K() {
  byte response[11];
  int val = readRS485Sensor(NPK_CMD, sizeof(NPK_CMD), response);
  if (val >= 0) return (response[7] << 8) | response[8];
  return 120;  // Fallback
}
